...
def _set_fastqs_and_check(
    ...
    try:
        ...
        paired_end = itsxpress._is_paired(
            fastq=fastq, fastq2=fastq2, single_end=single_end
        )
    except (NotADirectoryError, FileNotFoundError):
        raise ValueError("There is a problem with the fastq file(s) you selected")

    if paired_end:
        sobj = itsxpress.SeqSamplePairedNotInterleaved(
            fastq=fastq,
            fastq2=fastq2,
            tempdir=tempdir,
        ...
        sobj = itsxpress.SeqSampleNotPaired(fastq=fastq, tempdir=tempdir)
        return sobj
...
def main(
    ...
    if trim_ccs:
        ...
        marker_path = os.path.join(
            tempfile.gettempdir(), f"itsxpress_trim_ccs_{parent_pid}.tmp"
        )
