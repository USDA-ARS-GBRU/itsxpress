...
from qiime2.plugin import Plugin, Str, Choices, Int, Float, Range, Bool, Citations

from itsxpress.q2_itsxpress import trim_single, trim_pair, trim_pair_output_unmerged
...
plugin.methods.register_function(
    ...
    parameters={
        "region": Str % Choices(["ITS2", "ITS1", "ALL"]),
        "taxa": Str % Choices(taxaList),
        "threads": Int,
        "cluster_id": Float
        % Range(0.995, 1.0, inclusive_start=True, inclusive_end=True),
        "trim_ccs": Bool,
    },
    outputs=[("trimmed", SampleData[SequencesWithQuality])],
    input_descriptions={
        "per_sample_sequences": "The artifact that contains the sequence file(s)."
