...
def test_its_position_init():
    itspos = itsxpress.main.ItsPosition(
        os.path.join(TEST_DIR, "test_data", "ex_tmpdir", "domtbl.txt"), "ITS2"
    )
    exp1 = {
        "tlen": 341,
        "right": {"score": 59.1, "to_pos": 326, "from_pos": 282},
        "left": {"score": 52.2, "to_pos": 128, "from_pos": 84},
    }
...
def test_dedup():
    ...
    assert (
        dedup.matchdict["M02696:28:000000000-ATWK5:1:1101:11740:1800"]
        == "M02696:28:000000000-ATWK5:1:1101:10899:1561"
    )
    # Check that representative seqs are logged
    assert (
        dedup.matchdict["M02696:28:000000000-ATWK5:1:1101:23011:4341"]
        == "M02696:28:000000000-ATWK5:1:1101:23011:4341"
    )
...
def test_dedup_create_trimmed_seqs():
    ...
    # Check non gzipped
    dedup.create_trimmed_seqs(
        os.path.join(tf, "testout.fastq"),
        gzipped=False,
        zstd_file=False,
        wri_file=True,
        itspos=itspos,
...
def test_dedup_create_trimmed_seqs_gzipped():
    ...
    # Check gzipped
    dedup.create_trimmed_seqs(
        os.path.join(tf, "testout.fastq.gz"),
        gzipped=True,
        zstd_file=False,
        wri_file=True,
        itspos=itspos,
...
def test_dedup_create_trimmed_seqs_zst():
    ...
    dedup.create_trimmed_seqs(
        os.path.join(tf, "testout.fastq.zst"),
        gzipped=False,
        zstd_file=True,
        wri_file=True,
        itspos=itspos,
...
def test_seq_sample_paired_not_interleaved():
    ...
    sobj = itsxpress.main.SeqSamplePairedNotInterleaved(
        fastq=fastq, tempdir=tf, fastq2=fastq2
    )
...
def test_get_paired_seq_generator():
    ...
    fastq = os.path.join(TEST_DIR, "test_data", "4774-1-MSITS3_R1.fastq")
    fastq2 = os.path.join(TEST_DIR, "test_data", "4774-1-MSITS3_R2.fastq")
    dedup = itsxpress.main.Dedup(
        uc_file=uc, rep_file=rep, seq_file=seq, fastq=fastq, fastq2=fastq2
    )
    itspos = itsxpress.main.ItsPosition(
        os.path.join(TEST_DIR, "test_data", "ex_tmpdir", "domtbl.txt"), "ITS2"
    )
...
def test_coordinate_zero_not_falsy():
    ...
    res_gen = list(
        dedup._get_trimmed_seq_generator(iter(seqgen), itspos, wri_file=True)
    )
...
def test_trim_ccs_stitching():
    ...
    res_gen = dedup._get_trimmed_seq_generator(
        iter(seqgen), itspos, wri_file=True, trim_ccs=True
    )
    res_list = list(res_gen)
...
def test_create_runtime_hmm():
    ...
    try:
        ...
        hmm_path_its1 = create_runtime_hmm(
            taxa="Fungi", region="ITS1", tempdir=temp_dir
        )
        ...
        # Test taxa="All"
        hmm_path_taxa_all = create_runtime_hmm(
            taxa="All", region="ITS2", tempdir=temp_dir
        )
